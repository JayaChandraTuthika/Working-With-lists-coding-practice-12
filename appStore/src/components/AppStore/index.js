...
class AppStore extends Component {
  ...
  render() {
    ...
    const selectedAppsList = this.getSelectedCategoryApps(activeTabId)
    const filteredAppsList = selectedAppsList.filter(each =>
        ...
        <ul className="tabs-list-container">
          {tabsList.map(each => (
            <TabItem
              ...
              changeTabItem={this.onChangeTab}
              key={each.tabId}
            />
          ))}
        </ul>
